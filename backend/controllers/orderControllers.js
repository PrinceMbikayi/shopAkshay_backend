...
async function getSalesData(startDate, endDate) {
    let salesData = []; // Initialisez salesData ici
    ...
    try {
        ...
        console.error('Error during sales data aggregation:', error);
        throw new ErrorHandler('Could not fetch sales data', 500);
    }

    // Create a Map to store sales data and num of order by date
    const salesMap = new Map();
    let totalSales = 0; // Utilisez let au lieu de const
    ...
    const finalSalesData = datesBetween.map((date) => ({
        ...
        numOrders: (salesMap.get(date) || { numOrders: 0 }).numOrders, // Correction ici
    }));
